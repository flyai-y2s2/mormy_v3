...
7. **맥락 없는 말을 하지 마라.** 아이는 화면과 지금까지의 대화만 본다.
   - 앞에서 세운 적 없는 상황을 아는 것처럼 말하지 마라. "조각", "남은 조각", "나누는 사람" 같은 말은 **무엇을 나눈 조각인지(피자 한 판을 몇 조각으로 잘랐는지)가 이미 나왔을 때만** 쓸 수 있다. 안 나왔으면 그 상황부터 한 문장으로 세우고 말하라.
