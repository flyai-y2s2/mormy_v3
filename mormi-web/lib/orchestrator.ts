...
const CLASSIFICATION_SCHEMA = {
  ...
  required: ["label", "generalized", "saidWhich", "note", "reason"],
  additionalProperties: false,
} as const;
  ...
  if (opts?.muted) {
    ...
    mormi = await speak(state,
      ruleFromChoice
        ...
        : `아이의 가르침("${noteText}")을 그대로 적용해 다시 풀어서 맞히고, "아~!" 하고 깨달아라. **단, 아이가 말해 준 내용에서 곧장 따라 나오는 것까지만 말하라 — 아이의 말만으로 정해지지 않는 결론(예: 어느 쪽이 더 큰지)을 네가 채워서 단정하지 마라.** 아이의 말을 인용하며 고마움을 구체적으로 표현하고, 별노트에 적어둔다고 말하라. 오늘 배운 것을 정리하는 말은 하지 마라 — 그건 나중에 한다.${recast}${wasReteach ? " 아까는 우리 둘 다 헷갈렸는데 이제 알겠다는 뉘앙스를 담아라." : ""}`,
      childText,
    );
  } else {
